...
export class WebService {
  ...
  updateUser(id: string, editForm: any) {
    ...
    return this.http.put(
      `https://prod-09.uksouth.logic.azure.com/workflows/472ad5cd998d460f8dd06c894a62f5d0/triggers/manual/paths/invoke/rest/v1/user/${id}` +
        '?api-version=2016-10-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=GM345AOk8_MiefpdL5CHcJzvTcrrp3lJKtXU61kchNA',
      postData,
      {
        headers,
      }
  ...
  deleteUser(id: string) {
    ...
    return this.http.delete(
      `https://prod-31.uksouth.logic.azure.com/workflows/2b16dc1e7a3a4ac5a1f00391f724c2c7/triggers/When_a_HTTP_request_is_received/paths/invoke/rest/v1/user/${id}` +
  ...
  getDateRangeMeasurements(dateRange: string): Observable<any> {
    const headers = this.getHeader();
    const url =
      `https://prod-05.uksouth.logic.azure.com/workflows/66a00b40d0054679a525c37e14669a2a/triggers/manual/paths/invoke/rest/v1/measurementsrange/` +
  ...
  private buildThresholdFormData(thresholdForm: any, parameters: any): FormData {
    let formData = new FormData();
    formData.append('name', thresholdForm.name);
    const paramNames = [
      'temperature',
      'moisture',
      'ec',
    ...
    paramNames.forEach((param, index) => {
      formData.append(`${param}Min`, parameters[index].currentLowValue);
      formData.append(`${param}Max`, parameters[index].currentHighValue);
    });
  ...
  createThreshold(thresholdForm: any, parameters: any) {
    const headers = this.getHeader();
    let postData = this.buildThresholdFormData(thresholdForm, parameters);
  ...
  updateThreshold(id: string, thresholdForm: any, parameters: any) {
    ...
    const url = `https://prod-19.uksouth.logic.azure.com/workflows/647907c970e44c2396813349e08c2af0/triggers/manual/paths/invoke/rest/v1/threshold/${id}` + 
